...
def convert_json_to_junit_xml(json_path, xml_path):
    ...
    if not isinstance(payload, dict):
        raise RuntimeError(
            f"Invalid report format: expected JSON object at top-level in {json_path}"
        )

    if "results" not in payload:
        raise RuntimeError(
            f"Invalid report format: missing 'results' key in {json_path}"
        )
    ...
    if not isinstance(test_results, list):
        raise RuntimeError(
            f"Invalid report format: 'results' must be a list in {json_path}"
        )
    ...
    for test in test_results:
        ...
        for marker in test.get("markers", []):
            ET.SubElement(properties, "property", {"name": "marker", "value": marker})

        # Links
        for link in test.get("links", []):
            ET.SubElement(properties, "property", {"name": "link", "value": link})
        ...
        if test.get("screenshot"):
            ET.SubElement(
                properties,
                "property",
                {"name": "screenshot", "value": test["screenshot"]},
            )

        # Logs
        for log in test.get("logs", []):
            ET.SubElement(properties, "property", {"name": "log", "value": str(log)})
